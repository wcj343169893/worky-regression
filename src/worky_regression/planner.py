...
4. **單元的「前置 SQL」由框架自動補，你不用寫**：每個單元在 endpoints.yaml 宣告的 guards
   前置（如 J2 申請前清殘留已確認工作避開 30207/30229、J5/J6 打卡前對齊打卡碼與時段）
   會在該 transition 前自動物化成 db_exec。你只挑單元、排順序即可，**不要自己重寫這些前置 db_exec**。
   只有用例需要「非標準」的時間/狀態操弄（菜單前置沒涵蓋的）才插入 kind=db_exec。
5. **不要自己寫 expect 驗證**——框架會依 side_effects 自動補 push / DB 狀態檢查。
   你只需挑單元、排順序。
6. db_exec（你自己加的那種）修改 DB 後一般要 flush_cache=true（worky 有 memcached）。
7. **多夥伴/多身分用例**：job 系統有預設 `labor` 與三個具名夥伴 `labor1`/`labor2`/`labor3`
...
8. **頂層 vars**：招募人數用 `"vars": {{"job_recruit_count": 1}}`（多人申請只錄取部分時必設）；
   **若路徑含 J5/J6 打卡**，在 vars 帶 `"start_code": "000000"`、`"end_code": "000000"`
   （框架物化的打卡碼前置 SQL 會用這兩個值對齊 DB，request 也用同值；你只要宣告 vars）。
9. **不要自己寫 expect 驗證**——框架會依 side_effects 自動補 push / DB 狀態檢查。
...
def _guard_db_execs(name: str, bind: dict[str, Any] | None) -> list[dict[str, Any]]:
    """從 endpoints.yaml 單元的 guards 物化出該 transition 前置所需的 db_exec 步驟。

    guards[*].satisfy.db_exec 是「跑這步前必須先做的確定性 SQL」（單一真實來源），例：
      J2 申請前清殘留已確認工作（繞 30207/30229）、J5/J6 打卡前對齊打卡碼與時段。
    db_exec 不經 bind 解析，故這裡依該步 bind 對 SQL 模板做角色字面替換
    （bind={{labor: labor2}} → 把 `{{{{labor.` 換成 `{{{{labor2.`），讓前置作用在正確身分上。
    setup SQL 一律 expect_min_affected=0（可能 0 列，例如本來就沒有殘留工作）。
    """
    out: list[dict[str, Any]] = []
    for g in (unit_spec(name).get("guards") or []):
        sql = ((g.get("satisfy") or {}).get("db_exec") or "").strip()
        if not sql:
            continue
        for role, target in (bind or {}).items():
            sql = sql.replace(f"{{{{{role}.", f"{{{{{target}.")
        out.append({"db_exec": sql, "flush_cache": True, "expect_min_affected": 0})
    return out
...
def build_path(plan: TaskPlan) -> dict[str, Any]:
    ...
    for st in plan.steps:
        ...
        if last_txn_actor is not None and actor == last_txn_actor and not spaced:
            ...
            spaced = True
        # 物化該 transition 的 guards 前置 SQL（30207 清殘留、打卡碼/時段對齊…），單一真實來源。
        steps.extend(_guard_db_execs(name, bind))
        u = unit_spec(name)
