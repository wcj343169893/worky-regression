...
// token 數縮寫（12345 → 12.3k）；cost_usd → 預估金額（USD，極小額補到 4 位小數）。
const fmtTok = (n) => !n ? "0" : n >= 1000 ? `${(n / 1000).toFixed(1)}k` : `${n}`;
const fmtUsd = (c) => `$${(+c || 0).toFixed((+c || 0) < 1 ? 4 : 2)}`;
// 一條標記的「本次處理消耗」chip：總 token（in+out）+ 預估金額；無消耗（未處理）回空字串。
function costChip(m) {
  const tin = +m.tokens_in || 0, tout = +m.tokens_out || 0, usd = +m.cost_usd || 0;
  if (!tin && !tout && !usd) return "";
  return `<span class="pill pill-cost" title="輸入 ${tin.toLocaleString()} tok · 輸出 ${tout.toLocaleString()} tok · 預估 ${fmtUsd(usd)}">`
    + `🪙 ${fmtTok(tin + tout)} tok · ${fmtUsd(usd)}</span>`;
}
...
function statusSig(items) {
  return items.map((m) => `${m.id}:${m.status}:${m.resolved ? 1 : 0}:${m.result ? 1 : 0}:${(m.replies || []).length}:${m.cost_usd || 0}`).join("|");
}
